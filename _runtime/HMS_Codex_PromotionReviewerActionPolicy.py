...
def evaluate_action_policy(report, live_observation, *, busy=False):
    ...
    provenance_ok = bool(HEX64.fullmatch(evidence_digest) and HEX64.fullmatch(package_digest)
                         and HEX64.fullmatch(manifest_digest) and HEX64.fullmatch(trust_digest)
                         and HEX64.fullmatch(expected_trust) and trust_digest == expected_trust)
    ...
    common = (evidence_verified and provenance_ok and signature_ok and trust_ok and release_authority_ok
              and freshness_ok and idempotency_ok and trusted_live and not busy)
    ...
    if not release_authority_ok: reasons_out.append("SEALED_REVIEWER_RELEASE_AUTHORITY_REQUIRED")
    ...
    return {"product": "HMS-AI-ROUTER", "version": VERSION, "suite": "PROMOTION_REVIEWER_ACTION_POLICY",
        ...
        "gates": {"evidence": evidence_verified and provenance_ok, "signature": signature_ok,
            "reviewer_trust_authority": authority_ok, "reviewer_release_authority": release_authority_ok,
            "trust": trust_ok, "freshness": freshness_ok, "idempotency": idempotency_ok,
...
def synthetic_proof():
    ...
    release_authority = {"valid": True, "local_integrity_seal_valid": True, "packet_derived": False,
