...
def _verify_release_authority(*, ctl: PromotionWorkbenchController, authority_path: Path,
    ...
    package_sha = _sha_file(package_zip); manifest_sha = _sha_file(release_manifest)
    ...
    if package_sha != expected_package: raise ValueError("RELEASE_AUTHORITY_PACKAGE_SHA256_MISMATCH")
    if manifest_sha != expected_manifest: raise ValueError("RELEASE_AUTHORITY_MANIFEST_SHA256_MISMATCH")
    return authority
...
def import_for_review(*, state_dir: Path, packet: Path, package_zip: Path, release_manifest: Path,
    ...
    reviewed_release = _verify_release_authority(ctl=ctl, authority_path=release_authority_path,
        package_zip=package_zip, release_manifest=release_manifest, freshness_hours=release_authority_freshness_hours)
    report = ctl.ingest(
        packet,
        ...
        release_authority_freshness_hours=release_authority_freshness_hours,
    )
    ...
    if str(persisted_release.get("authority_sha256") or "").lower() != str(reviewed_release.get("authority_sha256") or "").lower():
...
def source_proof() -> dict:
    ...
    checks = {
        ...
        "controller_receives_release_authority_path": "release_authority_path=release_authority_path" in src,
        ...
        "release_authority_binding_reloaded_from_controller_seal": "SEALED_RELEASE_AUTHORITY_BINDING_PERSISTENCE_MISMATCH" in post_ingest,
    ...
    return {"product": PRODUCT, "version": VERSION, "suite": "REVIEWER_PACKET_IMPORT_SOURCE_PROOF",
        ...
        "real_packet_import_executed": False, "real_release_authority_verified": False,
...
def main() -> int:
    ...
    ap.add_argument("--proof", action="store_true"); ap.add_argument("--state-dir", default=""); ap.add_argument("--packet", default="")
    ap.add_argument("--package-zip", default=""); ap.add_argument("--release-manifest", default=""); ap.add_argument("--trust-authority", default="")
    ap.add_argument("--release-authority", default=""); ap.add_argument("--authority-freshness-hours", type=int, default=24)
    ap.add_argument("--release-authority-freshness-hours", type=int, default=168); ap.add_argument("--cockpit-baseline", default=COCKPIT_BASELINE)
    args = ap.parse_args()
    if args.proof:
        ...
        try:
            out = import_for_review(state_dir=Path(args.state_dir), packet=Path(args.packet), package_zip=Path(args.package_zip),
                release_manifest=Path(args.release_manifest), trust_authority=Path(args.trust_authority), release_authority_path=Path(args.release_authority),
                authority_freshness_hours=args.authority_freshness_hours, release_authority_freshness_hours=args.release_authority_freshness_hours,
                cockpit_baseline=args.cockpit_baseline); code = 0
        except Exception as exc:
            out = {"product": PRODUCT, "version": VERSION, "suite": "REVIEWER_PACKET_IMPORT", "verdict": "BLOCKED_FAIL_CLOSED",
                "error": type(exc).__name__, "detail": str(exc), "windows_runtime_certified": False,
                "production_score_promotion_eligible": False, "automatic_production_certification": False}; code = 2
