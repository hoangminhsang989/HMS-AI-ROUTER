...
def _verified_report_gate(report):
    ...
    required_digests = (
        "raw_packet_sha256", "package_zip_sha256", "release_manifest_sha256", "trust_snapshot_sha256",
...
class PromotionWorkbenchController:
    ...
    def _load_replay_registry(self):
        return self._load_sealed_json(
            self.replay_path, self.replay_seal_path,
    ...
    def ingest(self, packet_path, *, expected_package_sha256, expected_manifest_sha256, trust_authority_path,
        ...
        authority = trust_authority.load_and_verify_authority(
            Path(trust_authority_path), key_path=self.integrity_key_path,
        ...
        if report["real_packet_verified"]:
            ...
            for key, value in (("packet_digests", report["provenance"]["raw_packet_sha256"]), ("nonces", packet.get("nonce")),
                ...
                if value not in items:
                    items.append(value)
            # Fail closed: consume replay identifiers before publishing new review metadata.
    ...
    def record_decision(self, *, decision, reviewer_identity, reviewer_salt, lane, package_version,
        ...
        return {"reviewer_ref": ref, "decision_sha256": record["decision_sha256"], "epoch": record["epoch"],
                "decision": record["decision"], "lane": record["lane"],
                "observed_cockpit_baseline": record["cockpit_baseline"], "raw_reviewer_identity_stored": False}
    ...
    def state(self, *, package_version, manifest_sha256, baseline_at_open, baseline_before_final_review,
...
def synthetic_proof():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        base = {"source_classification": "REAL_EXTERNAL_WINDOWS_CODEX", "synthetic": False, "local_only": False,
            ...
            "release_manifest_sha256": "b" * 64, "cockpit_baseline": COCKPIT_BASELINE,
            "capture_utc": now.isoformat(), "nonce": "nonce-012345", "run_id": "run-01234567", "report_id": "report-012345",
        ...
        # Synthetic reviewer-side trust store fixture, persisted separately from the packet file.
        ...
        first = ctl.ingest(packet_path, expected_package_sha256="a" * 64, expected_manifest_sha256="b" * 64,
                           trust_authority_path=authority_path)
        ...
        try:
            forged_ctl.record_review_action(decision="APPROVE", reviewer_identity="reviewer-x",
                reviewer_salt="controller-proof-salt-01", lane="TERMINAL_PTY", package_version=VERSION,
                live_baseline_provider=lambda: COCKPIT_BASELINE)
        except ValueError:
            forged_blocked = True

        report_bytes = ctl.report_path.read_bytes()
        tampered_report = json.loads(report_bytes.decode("utf-8")); tampered_report["import_digest"] = "f" * 64
        ...
        tampered_replay = json.loads(replay_bytes.decode("utf-8")); tampered_replay["packet_digests"] = []
        ...
        rogue_authority = json.loads(authority_path.read_text("utf-8"))
        rogue_authority["authority"]["trust_snapshot_sha256"] = "c" * 64
        rogue_path = root / "rogue-authority.json"
        rogue_path.write_text(json.dumps(rogue_authority, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        rogue_authority_blocked = False
        try:
            PromotionWorkbenchController(root / "rogue-state").ingest(
                packet_path, expected_package_sha256="a" * 64, expected_manifest_sha256="b" * 64,
                trust_authority_path=rogue_path)
        except ValueError:
            rogue_authority_blocked = True

        provider_calls = []
        def frozen_provider():
            provider_calls.append("frozen"); return COCKPIT_BASELINE
        for lane in ("TERMINAL_PTY", "PROJECT_RESUME"):
            for identity in ("reviewer-a", "reviewer-b"):
                ctl.record_review_action(decision="APPROVE", reviewer_identity=identity,
                    reviewer_salt="controller-proof-salt-01", lane=lane, package_version=VERSION,
                    live_baseline_provider=frozen_provider)
        state = ctl.state(package_version=VERSION, manifest_sha256="b" * 64,
                          baseline_at_open=COCKPIT_BASELINE, baseline_before_final_review=COCKPIT_BASELINE)
        drift = ctl.record_review_action(decision="APPROVE", reviewer_identity="reviewer-a",
            reviewer_salt="controller-proof-salt-01", lane="TERMINAL_PTY", package_version=VERSION,
        ...
        return {"product": "HMS-AI-ROUTER", "version": VERSION,
                "suite": "WINDOWS_PROMOTION_WORKBENCH_CONTROLLER_PROOF",
                "verdict": "PASS" if n == len(tests) else "FAIL",
                "summary": {"pass": n, "fail": len(tests)-n, "total": len(tests)}, "tests": tests,
                "synthetic_fixture_only": True, "windows_runtime_certified": False,
