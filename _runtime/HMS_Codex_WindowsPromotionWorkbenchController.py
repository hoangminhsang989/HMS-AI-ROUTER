...
from HMS_Codex_ExternalWindowsReviewPacketIngest import COCKPIT_BASELINE, VERSION, ARTIFACT_BINDING_SCHEMA, verify_packet
...
def _verified_report_gate(report):
    """Require full cryptographic + reviewer trust + release identity before any ledger mutation."""
    ...
    if release.get("valid") is not True or release.get("local_integrity_seal_valid") is not True: reasons.append("SEALED_REVIEWER_RELEASE_AUTHORITY_REQUIRED")
    if release.get("packet_derived") is not False: reasons.append("REVIEWER_RELEASE_AUTHORITY_PACKET_DERIVED")
    if release.get("local_artifact_hashed_at_capture") is not False: reasons.append("REVIEWER_RELEASE_AUTHORITY_SELF_HASHED")
    ...
    required_digests = ("raw_packet_sha256", "package_zip_sha256", "release_manifest_sha256", "trust_snapshot_sha256",
                        ...
                        "source_certification_report_sha256", "source_artifact_package_sha256", "source_artifact_manifest_sha256")
    ...
    if provenance.get("source_artifact_binding_schema") != ARTIFACT_BINDING_SCHEMA: reasons.append("PROVENANCE_ARTIFACT_BINDING_SCHEMA_INVALID")
    ...
    if str(provenance.get("source_artifact_package_sha256") or "").lower() != package_digest: reasons.append("PROVENANCE_SOURCE_PACKAGE_MISMATCH")
    if str(provenance.get("source_artifact_manifest_sha256") or "").lower() != manifest_digest: reasons.append("PROVENANCE_SOURCE_MANIFEST_MISMATCH")
    ...
    if HEX64.fullmatch(str(release.get("authority_sha256") or "").lower()) is None: reasons.append("REVIEWER_RELEASE_AUTHORITY_DIGEST_INVALID")
    if str(release.get("package_zip_sha256") or "").lower() != package_digest: reasons.append("REVIEWER_RELEASE_AUTHORITY_PACKAGE_MISMATCH")
    if str(release.get("release_manifest_sha256") or "").lower() != manifest_digest: reasons.append("REVIEWER_RELEASE_AUTHORITY_MANIFEST_MISMATCH")
    if GIT_HEX.fullmatch(str(release.get("source_commit_sha") or "").lower()) is None: reasons.append("REVIEWER_RELEASE_AUTHORITY_SOURCE_COMMIT_INVALID")
    if GIT_HEX.fullmatch(str(release.get("source_tree_sha") or "").lower()) is None: reasons.append("REVIEWER_RELEASE_AUTHORITY_SOURCE_TREE_INVALID")
    ...
    if not isinstance(case_sources, list) or len(case_sources) != 1 or str(case_sources[0]).lower() != str(provenance.get("source_certification_report_sha256") or "").lower(): reasons.append("PROVENANCE_CASE_SOURCE_REPORT_INVALID")
    if not isinstance(case_packages, list) or case_packages != [package_digest]: reasons.append("PROVENANCE_CASE_SOURCE_PACKAGE_INVALID")
    if not isinstance(case_manifests, list) or case_manifests != [manifest_digest]: reasons.append("PROVENANCE_CASE_SOURCE_MANIFEST_INVALID")
...
class PromotionWorkbenchController:
    ...
    def ingest(self, packet_path, *, expected_package_sha256, expected_manifest_sha256, trust_authority_path,
               release_authority_path, current_cockpit_baseline=COCKPIT_BASELINE,
               authority_freshness_hours=24, release_authority_freshness_hours=168):
        ...
        release = release_authority.load_and_verify_authority(Path(release_authority_path), key_path=self.integrity_key_path,
        ...
        expected_package = str(expected_package_sha256 or "").lower(); expected_manifest = str(expected_manifest_sha256 or "").lower()
        ...
        with _exclusive_ingest_lock(self.ingest_lock_path):
            ...
            report = verify_packet(packet, raw_packet_sha256=hashlib.sha256(raw).hexdigest(),
                expected_package_sha256=expected_package, expected_manifest_sha256=expected_manifest,
            ...
            if report["real_packet_verified"]:
                ...
                report["reviewer_release_authority"] = {
                    "valid": True, "authority_sha256": release.get("authority_sha256", ""),
...
def _proof_packet(now, h):
    ...
    base = {"source_classification": "REAL_EXTERNAL_WINDOWS_CODEX", "synthetic": False, "local_only": False,
        ...
        "case_results": [{"case_id": cid, "status": "PASS", "report_sha256": h(cid),
            "source_report_sha256": source_ref, "source_package_zip_sha256": pkg,
            "source_release_manifest_sha256": man} for cid in REQUIRED_RUNTIME_CASE_IDS]}
...
def synthetic_proof():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        release_capture = release_authority.capture_authority(package_zip_sha256="a" * 64, release_manifest_sha256="b" * 64,
            source_commit_sha="1" * 40, source_tree_sha="2" * 40, output_path=release_path, key_path=ctl.integrity_key_path)

        first = ctl.ingest(packet_path, expected_package_sha256="a" * 64, expected_manifest_sha256="b" * 64,
            trust_authority_path=authority_path, release_authority_path=release_path)
        ...
        no_release_ctl = PromotionWorkbenchController(root / "no-release-state"); no_release_blocked = False
        no_release_trust = root / "no-release-trust.json"
        trust_authority.capture_authority(trust_file, no_release_trust, no_release_ctl.integrity_key_path)
        try:
            no_release_ctl.ingest(packet_path, expected_package_sha256="a" * 64, expected_manifest_sha256="b" * 64,
                trust_authority_path=no_release_trust, release_authority_path=root / "missing-release.json")
        ...
        release_tamper = json.loads(report_bytes.decode("utf-8")); release_tamper["reviewer_release_authority"]["package_zip_sha256"] = "9" * 64
        ctl.report_path.write_text(json.dumps(release_tamper, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"); release_report_tamper_blocked = False
        try: ctl.load_verified_report()
        except ValueError: release_report_tamper_blocked = True
        ctl.report_path.write_bytes(report_bytes)
        ...
        try:
            PromotionWorkbenchController(root / "rogue-state").ingest(packet_path, expected_package_sha256="a" * 64,
                expected_manifest_sha256="b" * 64, trust_authority_path=rogue_path, release_authority_path=release_path)
        except ValueError: rogue_authority_blocked = True

        rogue_release = json.loads(release_path.read_text("utf-8")); rogue_release["authority"]["package_zip_sha256"] = "9" * 64
        rogue_release_path = root / "rogue-release.json"; rogue_release_path.write_text(json.dumps(rogue_release, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        rogue_release_blocked = False
        try:
            PromotionWorkbenchController(root / "rogue-release-state").ingest(packet_path, expected_package_sha256="a" * 64,
                expected_manifest_sha256="b" * 64, trust_authority_path=authority_path, release_authority_path=rogue_release_path)
        except ValueError: rogue_release_blocked = True

        concurrent_ctl = PromotionWorkbenchController(root / "concurrent-state")
        concurrent_authority = root / "concurrent-authority.json"; concurrent_release = root / "concurrent-release.json"
        trust_authority.capture_authority(trust_file, concurrent_authority, concurrent_ctl.integrity_key_path)
        release_authority.capture_authority(package_zip_sha256="a" * 64, release_manifest_sha256="b" * 64,
            source_commit_sha="1" * 40, source_tree_sha="2" * 40, output_path=concurrent_release, key_path=concurrent_ctl.integrity_key_path)
        ...
        def import_worker():
            try:
                ...
                r = concurrent_ctl.ingest(packet_path, expected_package_sha256="a" * 64, expected_manifest_sha256="b" * 64,
                    trust_authority_path=concurrent_authority, release_authority_path=concurrent_release)
        ...
        checks = {
            ...
            "persisted_report_passes_full_gate": _verified_report_gate(persisted)["valid"],
            "release_authority_persisted_in_verified_report": persisted["reviewer_release_authority"]["authority_sha256"] == release_capture["authority_sha256"],
            ...
            "missing_release_authority_blocks_ingest": no_release_blocked,
            ...
            "release_binding_tamper_rejected_by_local_seal": release_report_tamper_blocked,
            ...
            "tampered_release_authority_rejected": rogue_release_blocked,
