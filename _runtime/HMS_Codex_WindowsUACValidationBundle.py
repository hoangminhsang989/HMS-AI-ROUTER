...
def run_case(session_path: Path, output: Path, case_type: str) -> dict[str, Any]:
    ...
    reasons = _validate_session_runtime(session)
    if reasons:
        result = {"verdict": "BLOCKED", "reasons": reasons, "case_type": case_type, "effects_executed": False}
        _atomic_json(output, result)
        return result

    captured_utc = _utc_text(_utcnow())
    interactive = harness.interactive_report(harness.ACKNOWLEDGEMENT)
    ...
    report = {
        ...
        "product": PRODUCT,
        "version": VERSION,
        "suite": "WINDOWS_UAC_RECOVERY_VALIDATION_CASE",
        "case_type": case_type,
        ...
        "source_bundle_sha256": session["source_bundle_sha256"],
...
def _verify_pair_data(session: dict[str, Any], cancel: dict[str, Any], close: dict[str, Any]) -> dict[str, Any]:
    ...
    host_ref = str(session.get("host_ref") or "")
    source_sha = str(session.get("source_bundle_sha256") or "")

    for label, report, case_type, expected in (
        ("CANCEL", cancel, "cancel", EXPECTED_CANCEL),
        ("CLOSE", close, "close", EXPECTED_CLOSE),
    ):
    ...
    try:
        cancel_time = _parse_utc(cancel.get("captured_utc"))
        close_time = _parse_utc(close.get("captured_utc"))
        created = _parse_utc(session.get("created_utc"))
        expires = _parse_utc(session.get("expires_utc"))
    ...
    return {
        "verdict": "PASS_BOUNDED_UAC_RECOVERY_PAIR" if not reasons else "BLOCKED_FAIL_CLOSED",
        "valid": not reasons,
        "reasons": sorted(set(reasons)),
        "session_id": session_id,
        "host_ref": host_ref,
        "source_bundle_sha256": source_sha,
        "cancel_report_sha256": str(cancel.get("report_sha256") or ""),
        "close_report_sha256": str(close.get("report_sha256") or ""),
        "cryptographic_attestation": False,
        "production_evidence_eligible": False,
        "canonical_seven_case_certification": False,
        "windows_runtime_certified": False,
...
def synthetic_proof() -> dict[str, Any]:
    ...
    close = {
        ...
        "interactive": {"verdict": EXPECTED_CLOSE, "same_token_replay_blocked": True, "uac_prompt_executed": True,
                        "effects_executed": True, "closed_pid_count": 2},
    }
    ...
    checks = {
        ...
        "session_requires_readonly_preflight": "harness.preflight_report()" in impl_src and "interactive_prerequisite_met" in impl_src,
    ...
    return {
        "product": PRODUCT, "version": VERSION, "suite": "WINDOWS_UAC_VALIDATION_BUNDLE_PROOF",
...
def main() -> int:
    ...
    cancel = sub.add_parser("run-cancel")
    cancel.add_argument("--session", required=True); cancel.add_argument("--output", required=True)
    close = sub.add_parser("run-close")
    close.add_argument("--session", required=True); close.add_argument("--output", required=True)
    verify = sub.add_parser("verify")
    verify.add_argument("--session", required=True); verify.add_argument("--cancel", required=True); verify.add_argument("--close", required=True)
    ...
    if args.command == "proof":
        ...
        result = session_init(Path(args.output))
    elif args.command == "run-cancel":
        result = run_case(Path(args.session), Path(args.output), "cancel")
    elif args.command == "run-close":
        result = run_case(Path(args.session), Path(args.output), "close")
    else:
        result = verify_pair(Path(args.session), Path(args.cancel), Path(args.close), Path(args.output) if args.output else None)
