...
from HMS_Codex_ExternalWindowsReviewPacketIngest import COCKPIT_BASELINE, ARTIFACT_BINDING_SCHEMA, verify_packet
...
EV="e"*64; MAN="b"*64; PKG="a"*64; SRC="c"*64
...
def _packet(now:datetime)->dict:
    base={"source_classification":"REAL_EXTERNAL_WINDOWS_CODEX","synthetic":False,"local_only":False,
          ...
          "case_results":[{"case_id":cid,"status":"PASS","report_sha256":_sha(cid),"source_report_sha256":SRC,
                           "source_package_zip_sha256":PKG,"source_release_manifest_sha256":MAN}
...
def _with_reviewer_authorities(report:dict)->dict:
    out=json.loads(json.dumps(report)); provenance=out.get("provenance") or {}; trust=str(provenance.get("expected_trust_snapshot_sha256") or "")
    ...
    out["reviewer_release_authority"]={"valid":True,"authority_sha256":"8"*64,"package_zip_sha256":PKG,
        "release_manifest_sha256":MAN,"source_commit_sha":"1"*40,"source_tree_sha":"2"*40,
...
def synthetic_e2e_fixtures():
    ...
    missing_binding=json.loads(json.dumps(packet)); missing_binding.pop("source_artifact_binding",None); binding_fail=_verify(missing_binding,now,expected_anchor=approved_anchor)
    case_splice=json.loads(json.dumps(packet)); case_splice["case_results"][0]["source_package_zip_sha256"]="f"*64; case_splice_fail=_verify(case_splice,now,expected_anchor=approved_anchor)
