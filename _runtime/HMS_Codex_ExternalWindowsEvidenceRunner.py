...
REASON_VI = {
    ...
    "CERTIFICATE_INSPECT_SCRIPT_MISSING": "Thiếu PowerShell certificate preflight helper.",
    "CERTIFICATE_PREFLIGHT_FAILED": "Certificate được chọn không đạt preflight read-only.",
    "CERTIFICATE_NOT_ACTIVE_IN_TRUST_STORE": "Certificate được chọn không phải ACTIVE pin trong trust store hiện hành.",
...
def _codex_probe(codex: str = ""):
    ...
    if not exe:
        return {"ok": False, "reason": "CODEX_CLI_NOT_FOUND", "path": ""}
    try:
...
def _parse_case_specs(values):
    ...
    for raw in values or []:
        if "=" not in raw:
            raise ValueError("case report must use CASE_ID=PATH")
        cid, p = raw.split("=", 1); cid = cid.strip(); p = p.strip()
        if cid in out:
            raise ValueError("duplicate case report: " + cid)
        out[cid] = Path(p)
    return out
...
def preflight(args):
    ...
    if not is_windows:
        reasons.append("WINDOWS_TARGET_REQUIRED")
    codex = _codex_probe(args.codex)
    if not codex["ok"]:
        reasons.append(codex["reason"])
    ...
    try:
        ...
        specs = {}; reasons.append("CASE_SPEC_INVALID:" + str(exc))
    matrix = validate_case_ids(specs.keys())
    if not matrix["valid"]:
        reasons.append("EXACT_SEVEN_CASE_REPORTS_REQUIRED")
    missing_files = [cid for cid, p in specs.items() if not p.is_file()]
    if missing_files:
        reasons.append("CASE_REPORT_FILES_MISSING")
    ...
    if not trust_path or not trust_path.is_file():
        reasons.append("TRUST_STORE_MISSING")
    else:
        try:
    ...
    if not sign_script.is_file():
        reasons.append("CERTIFICATE_SIGN_SCRIPT_MISSING")
    if not inspect_script.is_file():
        reasons.append("CERTIFICATE_INSPECT_SCRIPT_MISSING")
    crypto_ready = importlib.util.find_spec("cryptography") is not None
    if not crypto_ready:
        reasons.append("CRYPTOGRAPHY_REQUIRED")

    cert_check = {}
    if is_windows and THUMBPRINT.fullmatch(thumbprint) and inspect_script.is_file() and crypto_ready:
        try:
            cert_check = certificate_preflight.certificate_preflight(thumbprint, inspect_script)
            selected_sha = str(cert_check.get("certificate_sha256") or "").lower()
            if active_certificates and selected_sha not in active_certificates:
                reasons.append("CERTIFICATE_NOT_ACTIVE_IN_TRUST_STORE")
        except Exception as exc:
            reasons.append("CERTIFICATE_PREFLIGHT_FAILED:" + str(exc))

    reasons = sorted(set(reasons))
    return {
        "product": "HMS-AI-ROUTER", "version": VERSION, "suite": "EXTERNAL_WINDOWS_EVIDENCE_RUNNER_PREFLIGHT",
        "ready": not reasons, "reasons": reasons, "reasons_vi": [_reason_vi(x) for x in reasons],
        ...
        "certificate_preflight_ready": cert_check.get("ready") is True,
        "certificate_not_after_utc": cert_check.get("not_after_utc", ""),
        "certificate_thumbprint_ref": attestation_signer.safe_ref(thumbprint) if thumbprint else "",
        "certificate_sign_script": str(sign_script), "certificate_inspect_script": str(inspect_script),
        "creates_synthetic_evidence": False, "windows_runtime_certified": False,
...
def _validate_case_report(cid, path):
    ...
    if obj.get("synthetic") is not False:
        reasons.append("CASE_SYNTHETIC_OR_UNMARKED")
    if obj.get("local_only") is not False:
        reasons.append("CASE_LOCAL_ONLY_OR_UNMARKED")
    if str(obj.get("target_os") or "").lower() != "windows":
...
def build_packet(args):
    ...
    if str(pf.get("selected_certificate_sha256") or "").lower() not in trust_store.trusted_certificate_sha256(store):
        raise RuntimeError("CERTIFICATE_NOT_ACTIVE_IN_TRUST_STORE")

    packet = {
        "source_classification": SOURCE_CLASSIFICATION, "synthetic": False, "local_only": False,
    ...
    sign_script = Path(args.certificate_sign_script) if args.certificate_sign_script else DEFAULT_SIGN_SCRIPT
    packet["signer"] = external_certificate_signer.certificate_sign(
        signing_payload(packet), _normalize_thumbprint(args.certificate_thumbprint), sign_script)
    if str(packet["signer"].get("certificate_sha256") or "").lower() != str(pf.get("selected_certificate_sha256") or "").lower():
        raise RuntimeError("CERTIFICATE_CHANGED_AFTER_PREFLIGHT")

    raw = _stable_bytes(packet)
    check = verify_packet(
        packet, raw_packet_sha256=hashlib.sha256(raw).hexdigest(),
        expected_package_sha256=packet["package_zip_sha256"],
        expected_manifest_sha256=packet["release_manifest_sha256"],
        expected_trust_snapshot_sha256=snapshot["trust_snapshot_sha256"],
        current_cockpit_baseline=args.cockpit_baseline)
    if not check["real_packet_verified"]:
        raise ValueError("packet rejected by ingest gate: " + ",".join(check["reasons"]))
    out = Path(args.output); out.parent.mkdir(parents=True, exist_ok=True); out.write_bytes(raw)
    return {
        "product": "HMS-AI-ROUTER", "version": VERSION, "suite": "EXTERNAL_WINDOWS_EVIDENCE_RUNNER",
        ...
        "ingest_precheck": "PASS", "windows_runtime_certified": False,
...
def main():
    ...
    ap.add_argument("--output", default="HMS_EXTERNAL_WINDOWS_CODEX_REVIEW_PACKET.json")
    a = ap.parse_args()
    try:
