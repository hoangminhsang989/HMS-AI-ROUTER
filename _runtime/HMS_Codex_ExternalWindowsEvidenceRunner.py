...
import argparse, hashlib, json, platform, secrets, shutil, subprocess
from datetime import datetime, timezone
from pathlib import Path

from HMS_Codex_WindowsRuntimeCaseContract import REQUIRED_RUNTIME_CASE_IDS, validate_case_ids
from HMS_Codex_ExternalWindowsReviewPacketIngest import (
    COCKPIT_BASELINE,
...
def preflight(args):
    ...
    signer=Path(args.signer_json) if args.signer_json else None
    trust=Path(args.trust_json) if args.trust_json else None
...
def build_packet(args):
    ...
    signer=_read_json(Path(args.signer_json)); trust=_read_json(Path(args.trust_json))
    packet={"source_classification":SOURCE_CLASSIFICATION,"synthetic":False,"local_only":False,
            ...
            "report_id":"report-"+secrets.token_hex(16),
            "signer":signer,"trust_snapshot":trust,
    ...
    return {"product":"HMS-AI-ROUTER","version":VERSION,"suite":"EXTERNAL_WINDOWS_EVIDENCE_RUNNER",
            ...
            "package_zip_sha256":packet["package_zip_sha256"],
...
def main():
    ...
    ap.add_argument("--signer-json",default="")
    ap.add_argument("--trust-json",default="")
    ...
    a=ap.parse_args()
    try:
        out=preflight(a) if a.preflight else build_packet(a)
