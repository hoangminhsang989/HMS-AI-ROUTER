...
def _base_packet(now: datetime, *, seed: str = "base", case_ids: list[str] | tuple[str, ...] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "case_results": [
            {"case_id": cid, "status": "PASS", "report_sha256": _sha(f"{seed}:{i}:{cid}")}
...
def _signed(base: dict[str, Any]) -> dict[str, Any]:
    ...
    # Synthetic cryptographic fixture marker is removed only so ingest can exercise the
    # downstream field-specific fail-closed branches. This remains in-memory proof data.
...
def _exporter_cases() -> list[dict[str, Any]]:
    now = datetime(2026, 8, 24, 13, 0, tzinfo=timezone.utc)
    tests: list[dict[str, Any]] = []
...
def synthetic_proof() -> dict[str, Any]:
    tests = [*_ingest_cases(), *_exporter_cases()]
